...
class RoundedPanel(tk.Frame):
    """Abgerundeter Container mit sichtbarem Rand (Pillen-Look), z.B. fuer die Topbar."""

    def __init__(self, parent, height=64, radius=22, fill="#242424",
                 border_color="#3d3d3d", border_width=1, outer_bg=None):
        super().__init__(parent, bg=outer_bg or parent["bg"], bd=0, highlightthickness=0)
        self.fill = fill
        self.border_color = border_color
        self.border_width = border_width
        self.radius = radius

        self.canvas = tk.Canvas(self, height=height, bg=self["bg"], highlightthickness=0, bd=0)
    ...
    def _redraw(self, event=None):
        ...
        scale = 3
        sw, sh, sr = w * scale, h * scale, self.radius * scale
        img = make_rounded_image(sw, sh, sr, fill=hex_to_rgba(self.fill))
        draw = ImageDraw.Draw(img)
        bw = max(1, self.border_width * scale)
        inset = bw / 2
        draw.rounded_rectangle(
            [inset, inset, sw - 1 - inset, sh - 1 - inset],
            radius=sr, outline=hex_to_rgba(self.border_color), width=bw
        )
        img = img.resize((w, h), Image.LANCZOS)
        self._bgimg = ImageTk.PhotoImage(img)
        self.canvas.create_image(0, 0, image=self._bgimg, anchor="nw", tags="shape")
        self.canvas.tag_lower("shape")

        pad = 2
        self.canvas.coords(self.inner_window, pad, pad)
        self.canvas.itemconfig(self.inner_window, width=w - pad * 2, height=h - pad * 2)
...
def make_card_visual(width, height, radius, card_color, shadow_color, shadow_offset):
    """Zeichnet die Karte inkl. einem weichen (verwaschenen) Schlagschatten statt eines harten Duplikats."""
    scale = 2
    sw, sh, sr = width * scale, height * scale, radius * scale
    margin = 10 * scale
    soff = shadow_offset * scale
    ...
    sdraw.rounded_rectangle(
        [margin, margin + soff, sw - margin, sh - margin + soff],
        radius=sr, fill=hex_to_rgba(shadow_color, 140)
    )
    shadow_layer = shadow_layer.filter(ImageFilter.GaussianBlur(radius=7))
    img = Image.alpha_composite(img, shadow_layer)

    card_layer = Image.new("RGBA", (sw, sh), (0, 0, 0, 0))
    cdraw = ImageDraw.Draw(card_layer)
    cdraw.rounded_rectangle([margin, margin, sw - margin, sh - margin], radius=sr, fill=hex_to_rgba(card_color))
...
class AppCard(tk.Frame):
    ...
    def __init__(self, parent, game, launcher):
        ...
        self.inner = tk.Frame(self.canvas, bg=COLORS["card"], bd=0, highlightthickness=0)
        self._build_content()
    ...
    def _get_card_image(self, w, h):
        key = (w, h, self.hovered, self._shadow_offset)
        img = self._visual_cache.get(key)
        if img is None:
            fill = COLORS["card_hover"] if self.hovered else COLORS["card"]
            pil_img = make_card_visual(w, h, self.RADIUS, fill, COLORS["shadow"], self._shadow_offset)
            img = ImageTk.PhotoImage(pil_img)
            self._visual_cache[key] = img
        return img
    ...
    def _redraw(self, event=None):
        ...
        if w < 20 or h < 20:
            return
        self.canvas.delete("shape")
        img = self._get_card_image(w, h)
        self.canvas.create_image(0, 0, image=img, anchor="nw", tags="shape")
        self.canvas.tag_lower("shape")
...
class Skeet(BaseTk):
    ...
    # ---------- Aufbau ----------
    def _build_ui(self):
        topbar_wrap = RoundedPanel(self, height=64, radius=22, fill=COLORS["topbar"],
                                    border_color="#3d3d3d", border_width=1, outer_bg=COLORS["bg"])
        topbar_wrap.pack(side="top", fill="x", padx=20, pady=(18, 8))
        topbar = topbar_wrap.inner

        tk.Label(topbar, text="Skeet", bg=COLORS["topbar"], fg=COLORS["text"],
                 font=("Segoe UI", 15, "bold")).pack(side="left", padx=22)
        ...
        add_btn.pack(side="right", padx=(10, 22))
